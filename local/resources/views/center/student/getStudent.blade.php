			...
			<div class="app-content">
				...
				<div class="main-content" >
					<div class="wrap-content container" id="container">
						...
						<h1 style="font-size:18px;color:black">Course : {{$courseName->name}} | Batch : {{$batchName}}</h1>
						<form action="{!! Asset('center/studentAttendance/add') !!}" method="post" class="form-login">
						<input type="hidden" name="_token" value="{{ csrf_token() }}">
						<div class="row">
						<div class="col-md-10">
						<fieldset>
						<legend>
						Uncheck the student ame if its not present
						</legend>
						...
						<td width="20%"><b>View Detail</b></td>						
						</tr>
						<?php
						$i=0;
						...
						<input type="checkbox" id="checkbox_{{$i}}" value="1" name="chk_{{$student->id}}" checked>
						<label for="checkbox_{{$i}}">{{$student->first_name}} {{$student->last_name}} </label>
						
						</div>
						</td>
						<td width="20%"><a href="{{Asset('center/student/view/'.$student->id)}}" target="_blank" class="btn btn-info"><i class="fa fa-eye"></i> View Detail</a></td>
						</tr>
						<?php } ?>
						</table>
						<div class="form-group">						
						...
						<button type="button" class="btn btn-wide btn-info" onclick="window.location='../studentAttendance/add'">Go Back</button>
						</div>
						</div>						
						</fieldset>												
						</form>
